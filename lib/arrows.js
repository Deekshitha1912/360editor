...
// Text decal — a "3D embedded" text label, placed and rotated via the EXACT
// same imageLayer + 3-axis rotation mechanism as the floor decal above (same
// rotate_x/rotate_y/rotation columns, same on-canvas gizmo) — the only
// difference is the image itself is generated on the fly from the hotspot's
// own `label` text instead of being a fixed sprite. Reuses FLOOR_SIZE_MULTIPLIER
// (not a separate constant) since it's driven by the identical size/100
// world-scale convention.
//
// The SVG's viewBox width scales with the text length (a fixed-width box
// would either crop long labels or leave short ones swimming in empty
// space) while height stays fixed at 100 — callers combine that aspect
// ratio with the hotspot's own `size` to get final width/height, exactly
// the way FLOOR_SVG's fixed 1:1 square is used directly.
const TEXT_CHAR_W = 30
const TEXT_PAD_X = 24
export function textDecalSize(text) {
    const len = Math.max(1, (text || '').length)
    const width = TEXT_PAD_X * 2 + len * TEXT_CHAR_W
    return { width, height: 100 }
}

// escapeHtml-equivalent for the handful of XML-special characters that would
// otherwise break the SVG markup (or worse, let saved hotspot text inject
// markup into the generated image) — deliberately minimal, matching every
// other inline-SVG-from-user-text spot in this codebase.
function escXml(s) {
    return String(s ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&apos;' }[c]))
}

// Built fresh per hotspot (label/color can differ per instance, unlike every
// other ARROWS entry's fixed sprite) — called from both middle.jsx (live
// editor) and export.jsx (published tour, hand-synced copy below since that
// file ships as a plain string and can't import this module).
export function textDecalImage(text, color) {
    const { width, height } = textDecalSize(text)
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">`
        ...
        + `fill="${color || '#111111'}">${escXml(text || 'Text')}</text></svg>`
    return `data:image/svg+xml,${encodeURIComponent(svg)}`
}

// Palette-tile placeholder only — the real per-hotspot decal is always
// generated by textDecalImage() above once a label exists.
const TEXT_PLACEHOLDER_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">'
    + '<text x="50" y="62" text-anchor="middle" font-family="-apple-system,Segoe UI,Arial,sans-serif" '
    + 'font-size="52" font-weight="700" fill="#3730a3">Aa</text></svg>'
const TEXT_PLACEHOLDER_DATA_URI = `data:image/svg+xml,${encodeURIComponent(TEXT_PLACEHOLDER_SVG)}`
...
export const ARROWS = [
    ...
    { type: 'text',     jpg: TEXT_PLACEHOLDER_DATA_URI,       gif: TEXT_PLACEHOLDER_DATA_URI,       label: 'Text' },
