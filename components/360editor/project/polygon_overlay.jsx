...
function DetailFields({ detail, onChange }) {
    ...
    return (
        <div className="space-y-1.5">
            <label className="text-[10px] text-[#6b6b60] uppercase tracking-wider font-medium">Details</label>
            {rows.map((row, i) => (
                <div key={i} className="flex gap-1.5">
                    <input value={row.k} placeholder="Field"
                           onChange={e => commit(rows.map((r, j) => j === i ? { ...r, k: e.target.value } : r))}
                           className="w-[72px] shrink-0 h-7 bg-[#FAFAF7] border border-[#E2E2DA] rounded-lg px-2 text-[11px] text-[#1a1a18] focus:outline-none focus:border-[#3730a3] placeholder:text-[#9a9a8e]"/>
                    <input value={row.v} placeholder="Value"
                           onChange={e => commit(rows.map((r, j) => j === i ? { ...r, v: e.target.value } : r))}
                           className="flex-1 h-7 bg-[#FAFAF7] border border-[#E2E2DA] rounded-lg px-2 text-[11px] text-[#1a1a18] focus:outline-none focus:border-[#3730a3] placeholder:text-[#9a9a8e]"/>
                    <button onClick={() => commit(rows.filter((_, j) => j !== i))}
                            className="w-7 h-7 shrink-0 rounded-lg flex items-center justify-center text-[#9a9a8e] hover:text-red-500 hover:bg-red-50 transition-colors">
            ...
            {rows.length < MAX_DETAIL_KEYS && (
                <button onClick={() => commit([...rows, { k: '', v: '' }])}
                        className="text-[10.5px] font-semibold text-[#3730a3] hover:text-[#312e81] transition-colors">
                    + Add field
                </button>
            )}
        </div>
        ...
        <div className="absolute z-40 pointer-events-auto"
            ...
            <div className="bg-white/95 backdrop-blur-md rounded-xl border border-[#E2E2DA] shadow-[0_8px_32px_rgba(0,0,0,0.18)] overflow-hidden">
                ...
                <div className="flex items-center gap-2 px-3 py-2 bg-[#3730a3]/6 border-b border-[#E2E2DA]">
                    ...
                    <span className="text-[11px] font-bold text-[#3730a3] flex-1 truncate">{headerLabel}</span>
                    <button onClick={onCancel} className="text-[#9a9a8e] hover:text-[#1a1a18] transition-colors shrink-0">
                    ...
                    <div className="px-3 py-3 space-y-2.5 max-h-[360px] overflow-y-auto">
                        <div className="space-y-1">
                            ...
                            <input autoFocus value={state.label}
                                   ...
                                   className="w-full h-7 bg-[#FAFAF7] border border-[#E2E2DA] rounded-lg px-2.5 text-[12px] text-[#1a1a18] focus:outline-none focus:border-[#3730a3] placeholder:text-[#6b6b60]"/>
                        </div>
                        <div className="space-y-1">
                            <label className="text-[10px] text-[#6b6b60] uppercase tracking-wider font-medium">Status</label>
                                ...
                                {STATUS_OPTIONS.map(s => (
                                    <button key={s} onClick={() => onUpdate({ ...state, status: s })}
                                            className={`flex items-center gap-1.5 h-7 px-2.5 rounded-lg text-[11px] font-semibold border transition-colors ${
                                                state.status === s ? 'border-[#3730a3] bg-[#3730a3]/8 text-[#3730a3]' : 'border-[#E2E2DA] text-[#6b6b60] hover:border-[#3730a3]/40'
                                            }`}>
                            ...
                            <input value={STATUS_OPTIONS.includes(state.status) ? '' : state.status}
                                   ...
                                   className="w-full h-7 bg-[#FAFAF7] border border-[#E2E2DA] rounded-lg px-2.5 text-[11px] text-[#1a1a18] focus:outline-none focus:border-[#3730a3] placeholder:text-[#9a9a8e]"/>
                        </div>
                        <DetailFields detail={state.detail} onChange={detail => onUpdate({ ...state, detail })}/>
                        <div className="flex gap-1.5 pt-1">
                            <button onClick={onCancel}
                                    className="flex-1 h-7 text-[11px] rounded-lg border border-[#E2E2DA] text-[#6b6b60] hover:bg-[#F4F4EF] transition-colors">
                                Cancel
                            </button>
                            <button onClick={onSave} disabled={saving || !state.label.trim()}
                                    className="flex-1 h-7 text-[11px] rounded-lg bg-[#3730a3] text-white font-semibold hover:bg-[#312e81] disabled:opacity-40 transition-colors flex items-center justify-center gap-1">
                ...
                {state.mode === 'view' && state.polygon && (
                    <div className="px-3 py-3 space-y-2.5">
                        <div className="flex items-center gap-1.5 text-[11px] text-[#6b6b60]">
                            ...
                            <div className="space-y-1">
                                {Object.entries(state.polygon.detail).map(([k, v]) => (
                                    <div key={k} className="flex justify-between gap-2 text-[11px]">
                                        <span className="text-[#9a9a8e]">{k}</span>
                                        <span className="text-[#1a1a18] font-medium truncate">{String(v)}</span>
                                    </div>
                                ))}
                            </div>
                        )}
                        <div className="flex gap-1.5 pt-1">
                            <button onClick={onDelete} disabled={deleting}
                                    className="flex-1 h-7 text-[11px] rounded-lg border border-[#E2E2DA] text-red-500 hover:bg-red-50 disabled:opacity-40 transition-colors">
